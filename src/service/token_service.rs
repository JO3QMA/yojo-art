...
use crate::{models::{access_token::MiAccessToken, user::MiUser}, DBConnection, DataBase};
...
impl TokenPermission{
	pub fn is_allow(&self,key:PermissionKind)->bool{
		...
		match self{
			TokenPermission::Token(mi_access_token) => mi_access_token.permission.binary_search(&s).is_ok(),
	...
	pub async fn load_user(&self,db:&mut DBConnection<'_>)->Option<Cow<MiUser>>{
		match self{
			TokenPermission::Token(mi_access_token) => {
				MiUser::load_by_id(db,&mi_access_token.user_id).await.map(|t|Cow::Owned(t))
			},
