...
use crate::{
	DBConnection, DataBase, MisskeyConfig,
	models::{
		self,
...
impl DriveService {
	...
	pub async fn register_preflight(
		&self,
		...
		//#endregion
		let drive_folder =
			fetch_folder(&mut con, folder_id, user.as_ref().map(|u| u.id.as_str())).await;
	...
	pub async fn register_file(
		...
		if user_id.is_some() && !force {
			...
			if let Some(mut matched) = matched {
				...
				if sensitive && !matched.is_sensitive {
					...
					use crate::models::drive_file::drive_file::dsl::drive_file;
					use crate::models::drive_file::drive_file::dsl::*;
					let is_ok = diesel::update(drive_file.filter(id.eq(matched.id.as_str())))
						.set(isSensitive.eq(true))
						.execute(&mut con)
						.await
						.map_err(|e| {
							eprintln!("{:?}", e);
		...
		if maybe_sensitive
			&& profile
				.as_ref()
				.map(|profile| profile.auto_sensitive)
				.unwrap_or_default()
		{
			file.is_sensitive = true;
		}
		...
		let packed_file = self
			.pack(&mut con, &file, true, false, false, folder.as_ref(), user)
			.await;
		if let Some(user) = user.as_ref() {
			if let Some(packed_file) = packed_file.as_ref() {
				...
				let _ = self
					...
					.publish_main_stream(
						&user.id,
						Some(MainEventType::DriveFileCreated),
				...
				let _ = self
					...
					.publish_drive_stream(
						&user.id,
						Some(DriveEventType::FileCreated),
						Some(packed_file.clone()),
					)
					.await;
			};
	...
	pub async fn pack(
		...
		map.insert(
			"createdAt".into(),
			format!("{}", self.id_service.parse(file.id.as_ref())?.format("%+")).into(),
		);
		...
		if is_my_file {
			map.insert(
				"properties".into(),
				serde_json::to_value(file.properties.clone()).ok()?,
			);
		} else {
			map.insert(
				"properties".into(),
				serde_json::to_value(get_public_properties(file)).ok()?,
			);
		}
		if is_my_file {
		...
		if detail {
			if let Some(folder_id) = file.folder_id.as_ref() {
				let folder = if let Some(folder) = folder {
					...
					fetch_folder(con, Some(folder_id), file.user_id.as_deref())
						.await
						.map(|v| Cow::Owned(v))
				};
				map.insert(
					"folder".into(),
					self.pack_folder(con, folder.as_ref().map(|v| &**v), true)
						.await
						.unwrap_or(serde_json::Value::Null),
				);
			} else {
		...
		if with_user {
			...
			let pack_user = if let Some(user_id) = file.user_id.as_ref() {
				...
				match user.as_ref() {
					Some(user) => {
						self.user_service
							.pack(user, Some(user.id.as_ref()), &Default::default())
							.await
					}
					None => None,
				}
		...
		if detail {
			...
			loop {
				if let Some(folder_id) = parent_id {
					let parent = fetch_folder(
						con,
						Some(&folder_id),
						folder.user_id.as_ref().map(|x| x.as_str()),
					)
					.await;
					parent_id = parent
						.as_ref()
						.map(|v| v.parent_id.clone())
						.unwrap_or_default();
					match self.pack_folder0(con, parent.as_ref(), true).await {
						Some(v) => {
							parent_stack.push(v);
						}
						_ => {
			...
			fn insert(
				...
				mut arr: Vec<serde_json::Map<String, serde_json::Value>>,
			) {
				if arr.is_empty() {
					return;
				}
	...
	async fn pack_folder0(
		...
		map.insert(
			...
			format!(
				"{}",
				self.id_service.parse(folder.id.as_ref())?.format("%+")
			)
			.into(),
		);
		...
		if detail {
			...
			let folders_count: i64 = {
				...
				drive_folder
					.filter(parentId.eq(folder.id.as_str()))
					.count()
					.get_result(con)
					.await
					.map_err(|e| {
			...
			let files_count: i64 = {
				...
				use crate::models::drive_file::drive_file::dsl::*;
				drive_file
					.filter(folderId.eq(folder.id.as_str()))
					.count()
					.get_result(con)
					.await
					.map_err(|e| {
	...
	fn get_thumbnail_url(&self, file: &MiDriveFile) -> Option<String> {
		if file.mime_type.starts_with("video") {
			...
			&& file.user_host.is_some()
			&& self.config.media_proxy.is_some()
		{
	...
	fn get_public_url(&self, file: &MiDriveFile, mode: Option<&str>, ap: bool) -> String {
		// static = thumbnail
		...
		if ap && self.config.ap_file_base_url.is_some() {
			...
			match (
				reqwest::Url::from_str(ap_file_base_url.as_str()),
				reqwest::Url::from_str(url.as_str()),
			) {
...
pub async fn fetch_folder(
	...
	let res: MiDriveFolder = {
		...
		drive_folder
			...
			.select(MiDriveFolder::as_select())
			.first(con)
			.await
			.map_err(|e| {
				eprintln!("{:?}", e);
...
pub async fn calc_drive_usage_of(con: &mut DBConnection<'_>, user_id: &str) -> i64 {
	...
	*/
	let size_sum: Option<i64> = drive_file
		.filter(userId.eq(user_id))
		.filter(isLink.eq(false))
		.select(sum(size))
...
fn correct_filename(filename: &str, ext: Option<&str>) -> String {
	...
	const TARGET_EXTS_TO_SKIP: [&str; 7] = [".7z", ".bz2", ".gz", ".tar", ".tgz", ".xz", ".zip"];
