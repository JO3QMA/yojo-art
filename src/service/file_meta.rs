...
impl FileMetaService {
	pub(crate) fn new() -> Self {
		let model = nsfw::create_model(std::io::Cursor::new(include_bytes!(
			"../../assets/model.onnx"
		)));
	...
	pub async fn metadata(
		...
		let (maybe_sensitive, blurhash, thumbnail) = futures_util::join!(
			async {
			...
			tokio::task::spawn_blocking(move || {
				...
				let rgba = resize(
					cp,
					thumbnail_size.min(size.0),
					thumbnail_size.min(size.1),
					filter,
				)?;
	...
	pub async fn ffmpeg_metadata(
		...
		if let Ok(mut process) = tokio::process::Command::new(ffmpeg)
			...
			.spawn()
		{
			if let Some(mut stdout) = process.stdout.take() {
				let mut img = vec![];
				match stdout.read_to_end(&mut img).await {
					Err(e) => {
						println!("{:?}", e);
					}
					_ => {
						if let Ok(img) = image::load_from_memory(&img) {
							let info = self
								.metadata(
									img,
									sensitive_threshold,
									skip_sensitive_detection,
									thumbnail_size,
									config.thumbnail_quality,
									config.thumbnail_filter.into(),
								)
								.await;
...
fn examine(
	...
	use tract_data::tvec;
	let image: Tensor = ndarray::Array4::from_shape_fn(
		(1, resized.width() as usize, resized.height() as usize, 3),
