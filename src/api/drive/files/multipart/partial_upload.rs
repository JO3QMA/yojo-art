...
pub async fn post(
	...
	if buf.len() == 0 {
		...
		header.append("X-Error", "No Body".parse().unwrap());
	...
	if session.part_number == Some(0) {
		...
		session.ext = ext;
		session.upload_id = match ctx
			.bucket
			.initiate_multipart_upload(&session.s3_key, content_type)
			.await
		{
			Ok(imur) => Some(imur.upload_id),
			Err(e) => {
				eprintln!("{}:{} {:?}", file!(), line!(), e);
				return (StatusCode::INTERNAL_SERVER_ERROR).into_response();
			}
	...
	match ctx
		...
		.set_ex::<&String, String, ()>(
			&format!("multipartUpload:{}", hashed_sid),
	...
	tokio::runtime::Handle::current().spawn(async move {
			...
			.await
		{
			Ok(part) => {
				let _ = redis
					.set_ex::<String, String, ()>(temp_id, part.etag, 24 * 60 * 60)
					.await; //24時間後に失敗する
			}
			Err(e) => {
				eprintln!("{}:{} {:?}", file!(), line!(), e);
				//空文字列は失敗
				let _ = redis
					.set_ex::<&str, &str, ()>(temp_id.as_str(), "", 24 * 60 * 60)
					.await; //24時間後に失敗する
			}
		}
	});
