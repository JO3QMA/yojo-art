...
pub async fn post(
	...
	let permission = match req.i.take() {
		Some(token) => {
			ctx.token_service
				.get_permission(&crate::service::token_service::Token(token))
				.await
		}
	...
	let s3_key = format!(
		...
		req.ext.as_ref().map(|s| s.as_str()).unwrap_or("")
	);
	...
	let detected_name = percent_encoding::percent_encode(
		res.detected_name.as_bytes(),
	...
	let (raw_upload, (mut thumbnail_upload, mut info)) = futures_util::join!(
		ctx.bucket.put_object_with_metadata(
			&s3_key,
			&file_data,
			&content_type,
			content_md5.clone(),
	...
	let status =
		axum::http::StatusCode::from_u16(200).unwrap_or(axum::http::StatusCode::BAD_GATEWAY);
	(
		status,
		header,
		serde_json::to_string(&res.1.unwrap_or(serde_json::Value::Null)).unwrap_or_default(),
	)
		.into_response()
}
