...
use crate::{models::{access_token::MiAccessToken, drive_file::{self, MiDriveFile}, user::MiUser}, service::{event::DriveEventType, token_service::{Token, TokenPermission}}, Context, UploadSession};
...
pub async fn post(
	...
	let file :Option<MiDriveFile>={
		...
		drive_file.filter(id.eq(&parms.fileId)).select(MiDriveFile::as_select()).first(&mut con).await.map_err(|e|{
	...
	if let Some(thumbnail_access_key)=file.thumbnail_access_key.as_ref(){
		let _=ctx.bucket.delete_object(thumbnail_access_key).await.map_err(|e|{
			eprintln!("{}:{} {:?}",file!(),line!(),e);
		});
	}
	let deleted :Option<usize>={
		use crate::models::drive_file::drive_file::dsl::drive_file;
		use crate::models::drive_file::drive_file::dsl::*;
		diesel::delete(drive_file.filter(id.eq(&parms.fileId))).execute(&mut con).await.map_err(|e|{
			eprintln!("{}:{} {:?}",file!(),line!(),e);
		})
	}.ok();
	//TODO チャート
	if let Some(user_id)=file.user_id.as_ref(){
		let _=ctx.event_service.publish_drive_stream(user_id, Some(DriveEventType::FileDeleted), Some(serde_json::Value::String(file.id))).await;
	}
	//TODO モデログ
	match deleted{
		Some(0)=>{
